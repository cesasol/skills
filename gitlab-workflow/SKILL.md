...
description: >
  Guide for following GitLab workflows using the glab CLI. Use this skill whenever the user
  wants to create a merge request, update an MR description, break down an issue into tasks,
  start an MR from an issue, review an MR, check pipeline status, or follow any GitLab Flow
  practice. Triggers on phrases like "create MR", "make a merge request", "review this MR",
  "break this issue into tasks", "start working on issue #123", "check the pipeline",
  "what's the status of my MR", "create a task list", or any mention of GitLab workflow,
  GitLab Flow, or GLFM.
---

# GitLab Workflow

Follow GitLab Flow — a streamlined branching and collaboration strategy that integrates
feature-driven development, issue tracking, and CI/CD. This skill provides a prescriptive
workflow and hands-on recipes for common GitLab operations using the `glab` CLI.

**Prerequisite:** The `glab` skill must be available. Read its SKILL.md and the relevant
reference files from its `references/` directory when you need detailed command flags.
...
When a user asks to "work on" something, follow this sequence. Skip steps that are already
done, but always verify before skipping.
...
These are the most common tasks. For each, follow the detailed instructions.
