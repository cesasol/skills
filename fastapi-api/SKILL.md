---
name: fastapi-api
description: FastAPI API best practices. Use whenever the user mentions FastAPI, wants to build a REST API with Python, create FastAPI endpoints, routers, or middleware, structure a FastAPI project, write Pydantic schemas, set up dependency injection, handle errors, write tests for an API, configure a FastAPI app, or work with FastAPI + SQLModel / SQLAlchemy. Applies to both new projects and refactoring existing FastAPI code.
---
...
**Tip**: use the same path-variable name across routers when you want to reuse a dependency. E.g., `profile_id` in both `/profiles/{profile_id}` and `/creators/{profile_id}` lets you chain `valid_profile_id` → `valid_creator_id`.
...
If you use custom markers like `@pytest.mark.integration` or `@pytest.mark.slow`, register them in `pytest.ini` at the project root — not in `pyproject.toml`. Unregistered markers produce warnings and muddy CI output.
...
Don't mock the database in integration tests — mock/prod divergence inevitably fires in production. Use testcontainers, an ephemeral test schema, or a local dev DB with `dependency_overrides` for auth and external services only.
...
Now `uv run myapp users create --email a@b.com --username alice` works. CLI scripts import from `app.*` just like any other module — they share the same database session factory, config, and services. Don't duplicate logic between CLI and API.
